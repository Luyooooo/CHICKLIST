        ...
        try (Scanner scanner = new Scanner(new File(ARCHIVO_VENTAS))) {
            while (scanner.hasNextLine()) {
                ...
                if (datos.length == 5) {
                    ...
                    LocalDate fecha = LocalDate.ofEpochDay(Long.parseLong(datos[1]));//para no perderme xdddddddd
                    ...
                    if (cliente != null) {
                        ...
                        venta.marcarComoPagada(); // si estaba pagada
                        if (!pagada) {
                            venta = new Venta(cliente, productos); // crear sin marcar pagada
                        }
                        ventas.add(venta);
                    }
                }
